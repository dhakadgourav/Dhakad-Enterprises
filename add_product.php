...
$product_id = $name = $brand_name = $unit_price = $color = $supply_id =$fileToUpload= $categ_id = $quantity="";
$product_id_err = $name_err = $brand_name_err = $unit_price_err = $color_err = $supply_id_err = $categ_id_err = $quantity_err="";
...
//$imageProperties = getimageSize($_FILES['image']['tmp_name']);
    ...
    if(empty($product_id_err) && empty($name_err) && empty($brand_name_err) && empty($unit_price_err) && empty($color_err) && empty($supply_id_err) && empty($quantity_err) ){
        
        // Prepare an insert statement
        $sql = "INSERT INTO product (product_id,name,brand_name,unit_price,color,image, supply_id, categ_id,quantity) VALUES (".$product_id.", '".$name."', '".$brand_name."', ".$unit_price.", '".$color."','".$imgdata."',".$supply_id.", ".$categ_id.",".$quantity.")";
         
        //echo $sql;
         if(mysqli_query($link,$sql)){
                // Redirect to login page
            $res="update category set availability=availability+1 where category_id=".$categ_id;
        ...
        $ew="Update category set availability=availability+".$quantity." where category_id=".$categ_id;
        mysqli_query($link,$ew);
