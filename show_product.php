...
if(!(empty(trim( isset($_POST['number']) && $_POST['number']))) || !(empty(trim( isset($_POST['product_id']) && $_POST['product_id'])))){
$sql = "update product set quantity=quantity+".$_POST['number']." where product_id=".$_POST['product_id'];
    //echo $sql;
    if(mysqli_query($link,$sql))
      {
       ...
       $ew="Update category set availability=availability+".$_POST['number']." where category_id=".$_POST['categ_id'];
...
<br><br><br><br><br><br><br>
   <?php 
   $sql = "SELECT product_id, name, brand_name,unit_price, color, quantity,categ_id FROM product  ";
   //echo $sql;
...
<table class="table table-hover table-active" style="margin-left:300px">
  ...
  ';
    ...
    while($row = mysqli_fetch_assoc($result)) {
       // $id = $row["product_id"];
        //echo $id;
        echo "<tr><th>" . $row["product_id"]."</th><th>" . $row["name"]. "</th><th>". $row["brand_name"]."</th><th>" . $row["unit_price"]."</th><th><img src='view.php?id=".$row["product_id"]."' style='width:170px ; height:100px'></th><th>" . $row["quantity"]. "</th><td>

        <form action=\" ".htmlspecialchars($_SERVER["PHP_SELF"])." \" method=\"post\">
          <div class=\"col-sm-12\">
                <input type=\"text\" name=\"number\" value=0> 
                <input type=\"hidden\" name=\"product_id\" value=" . $row["product_id"].">
                <input type=\"hidden\" name=\"categ_id\" value=" . $row["categ_id"].">
